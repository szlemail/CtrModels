...
#variable
sparse_dim = train_xs.shape[1]
embed_dim = 6 * round(math.pow(sparse_dim, 0.25))
dense_dim = train_xd.shape[1]
latent_depth = 32
batch_size = 256
early_stop = True
tol = 3
l2 = 0.001
xs = tf.placeholder(tf.float32, shape = [None, feature_len], name = 'xs')
xd = tf.placeholder(tf.float32, shape = [None, feature_len], name = 'xd')
...
with tf.variable_scope("cross", reuse = tf.AUTO_REUSE):
    w0 = tf.Variable(tf.truncated_normal([sparse_dim, embed_dim], stddev = 0.1), name = 'w0')
    xe = tf.matmul(xs, w0)
    #xe = tf.contrib.layers.flatten(xe)

x0 = tf.concat([xd, xe], axis = 1) 
# input x should be 1 dim array. with is flattened
def cross(x):
    #weight
    with tf.variable_scope("cross", reuse = tf.AUTO_REUSE, regularizer = regularizer):
        b = tf.Variable(tf.truncated_normal([embed_dim + dense_dim], stddev = 0.1), name = 'b')
        w = tf.Variable(tf.truncated_normal([embed_dim + dense_dim], stddev = 0.1), name = 'w')
        x = tf.matmul(x,
    return logits
